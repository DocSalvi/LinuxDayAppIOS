...
@implementation HereTag

- (id) init: (GeoTag*) next:  (double) lat: (double) lon {
    return [ super init: next : lat : lon : 20 : 20 : 10 : 10 ];
}
...
- (void) scaledPaint: (CGContextRef) context: (int) x: (int) y {
    ...
    CGContextMoveToPoint(context, x+10, y+20); //start at this point
    CGContextAddLineToPoint(context, x+10, y); //draw to this point
    CGContextMoveToPoint(context, x, y+10); //start at this point
    CGContextAddLineToPoint(context, x+20, y+10); //draw to this point
    CGRect rectangle = CGRectMake(x,y,20,20);
